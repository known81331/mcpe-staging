...
#include "AuxDataTileItem.hpp"
#include "world/level/Level.hpp"
#include "world/tile/Tile.hpp"

AuxDataTileItem::AuxDataTileItem(int id) : Item(id)
{
	id += 256;
	m_tile = id;
...
std::string AuxDataTileItem::getDescriptionId()
{
	return Tile::tiles[m_tile]->getDescriptionId();
}

std::string AuxDataTileItem::getDescriptionId(ItemInstance* instance)
{
	return Tile::tiles[m_tile]->getDescriptionId();
}

int AuxDataTileItem::getLevelDataForAuxValue(int x) {
	return x;
}
